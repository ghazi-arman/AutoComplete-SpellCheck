...
void testStudent(std::string dict_filename) {
  ...
  std::cout << "\tTest 1: time taken: " << time_duration << " nanoseconds."
            << std::endl;
  ...
  results = dictionary_trie->predictCompletions("a", 10);
  time_duration = T.end_timer();
  std::cout << "\tTest 2: time taken: " << time_duration << " nanoseconds."
            << std::endl;
  ...
  time_duration = T.end_timer();
  std::cout << "\tTest 3: time taken: " << time_duration << " nanoseconds."
            << std::endl;
  ...
  time_duration = T.end_timer();
  std::cout << "\tTest 4: time taken: " << time_duration << " nanoseconds."
            << std::endl;
  ...
  time_duration = T.end_timer();
  std::cout << "\tTest 5: time taken: " << time_duration << " nanoseconds."
            << std::endl;
  ...
  if (response.compare("y") == 0) {
    ...
    while (std::getline(std::cin, prefix)) {
      ...
      std::cout << "\n\tUser Test: prefix= \"" << prefix
                << "\" num_completions= " << num_completions << std::endl;
      T.begin_timer();
      results = dictionary_trie->predictCompletions(prefix, num_completions);
      time_duration = T.end_timer();
      std::cout << "\tUser Test: time taken: " << time_duration
...
int main(int argc, char* argv[]) {
  ...
  benchdict(argv[1], argv[2], argv[3], argv[4]);
}
